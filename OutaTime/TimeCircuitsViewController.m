...
- (void)viewDidLoad
{
    ...
    //
    self.timeCircuitReadouts = [[NSDateFormatter alloc] init];
    self.timeCircuitReadouts.dateStyle = NSDateFormatterMediumStyle;
    self.timeCircuitReadouts.timeStyle = NSDateFormatterNoStyle;
    //
    
    
    // 3. This is a good place to initialize the objects that will be used later on.
    //    The date formatter object you created above needs to be instantiated.
    //
    ...
    self.timeCircuitReadouts.dateFormat = formatString;
    
    //
    // 5. The presentTimeLabel needs to be set to today's date. Use the dateFormatter object to do this.
    NSDate *today = [NSDate date];
    self.presentTimeLabel.text = [self.timeCircuitReadouts stringFromDate:today];
    ...
    self.speedLabel.text = [NSString stringWithFormat:@"%ld MPH", (long) self.currentSpeed];
       //
...
- (void)destinationDateWasChosen:(NSDate *)destinationDate
{
    ...
    self.destinationTimeLabel.text = [self.timeCircuitReadouts stringFromDate:destinationDate];
...
- (void)startTimer
{
    ...
    if (!self.speedTimer)
    {
        ...
        //
        self.speedTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(updateSpeed) userInfo:nil repeats:YES];
...
- (void)updateSpeed
{
    ...
    //
    if (self.currentSpeed != 88)//How to check the speed?
    {
        //
        // 18. If it's not yet set to 88, we want to increment the current speed variable by 1.
        self.currentSpeed = self.currentSpeed + 11;
